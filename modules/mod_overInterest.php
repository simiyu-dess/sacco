...
if($row_overd['ltrans_due'] < ($timestamp - $graceperiod) AND $row_overd['ltrans_fined'] != 1){
	...
	if($loandefault_inc < ($sav_balance - $_SESSION['set_msb'])){
		...
		user_id) VALUES ('$row_overd[cust_id]', '$row_overd[ltrans_id]', '$timestamp', '$loandefault_sav', '6', '',
		...
		$sql_fine_inc = "INSERT INTO incomes (cust_id, loan_id, ltrans_id, inctype_id, inc_amount, inc_date, inc_receipt, 
		inc_created, user_id) VALUES ('$row_overd[cust_id]', '$row_overd[loan_id]', '$row_overd[ltrans_id]', '5', 
		'$loandefault_inc', '$timestamp', '', '$timestamp', '$_SESSION[log_id]')";
