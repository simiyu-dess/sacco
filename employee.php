			...
			<form action="employee.php" method="post" onSubmit="return validate(this)">
				...
				<table id ="tb_fields" style="border-spacing:0.1em 1.25em;">
					<colgroup>
						...
						<?php if (isset($result_empl['empl_pic'])){ ?><img src="<?php echo $result_empl['empl_pic']; ?>" title="Employee\'s picture">
						<?php } else {
								if ($result_empl['emplsex_id'] == 2){ ?> <img src="ico/custpic_f.png" title="Upload new picture" />
								<?php } else ?><img src="ico/custpic_m.png" title="Upload new picture" />
						<?php } ?>
							</a>
										</td>
										<td>Empl. No:</td>
										<td><input type="text" name="empl_no" value="<?php echo $result_empl['empl_no']; ?>" tabindex=1 /></td>
										<td>Phone No:</td>
										<td><input type="text" name="empl_phone" value="<?php echo $result_empl['empl_phone']; ?>" tabindex=7 /></td>
									</tr>
						        <tr>
										<td>Name:</td>
										<td><input type="text" name="empl_name" value="<?php echo $result_empl['empl_name']; ?>" tabindex=2 /></td>
										<td>E-Mail:</td>
										<td><input type="text" name="empl_email" value="<?php echo $result_empl['empl_email']; ?>" placeholder="abc@xyz.com" tabindex=8 /></td>
									</tr>
									<tr>
										<td>Gender:</td>
										<td>
											<select name="emplsex_id" size="1" tabindex=3>';
								<?php while ($row_sex = mysqli_fetch_assoc($query_sex)){
									if($row_sex ['emplsex_id'] == $result_empl['emplsex_id']){
										?> <option selected value="<?php echo $row_sex['emplsex_id']; ?>"><?php echo $row_sex['emplsex_name']; ?></option>';
									<?php }
									 else ?><option value="<?php echo $row_sex['emplsex_id']; ?>"><?php echo $row_sex['emplsex_name']; ?></option>';
								<?php } ?>
								</select>
										</td>
										<td>Position:</td>
										<td><input type="text" name="empl_position" value="<?php echo $result_empl['empl_position']; ?>" placeholder="Job description" tabindex=9 /></td>
									</tr>
									<tr>
										<td>DoB:</td>
										<td><input type="text" id="datepicker" name="empl_dob" value="<?php echo date("d.m.Y",$result_empl['empl_dob']); ?>" placeholder="DD.MM.YYYY" tabindex=4 /></td>
										<td>Salary:</td>
										<td><input type="number" name="empl_salary" value="<?php echo $result_empl['empl_salary']; ?>" placeholder="<?php echo $_SESSION['set_cur']; ?>" tabindex=10 /></td>
									</tr>
									<tr>
										<td>Last updated:</td>
										<td><input type="text" disabled="diabled" value="<?php echo date("d.m.Y", $result_empl['empl_lastupd']); ?>" /></td>
										<td>Maritial Status:</td>
										<td>
											<select name="emplmarried_id" size="1" tabindex=5>';
											<?php while ($row_mstat = mysqli_fetch_assoc($query_mstat)){
												if($row_mstat ['emplmarried_id'] == $result_empl['emplmarried_id']){?>
													<option selected value="<?php echo $row_mstat['emplmarried_id']; ?>"><?php echo $row_mstat['emplmarried_status'];?></option>';
													<?php }
												 else ?> <option value="<?php echo $row_mstat['emplmarried_id']; ?>"><?php echo $row_mstat['emplmarried_status']; ?></option>';
											<?php } ?>
											 </select>
										</td>
										<td>Employm. Start:</td>
										<td><input type="text" name="empl_in" id="datepicker2" value="<?php echo date("d.m.Y", $result_empl['empl_in']); ?>" tabindex=11 /></td>
									</tr>
									<tr>
										<td>Username:</td>
										<td><input type="text" disabled="diabled" value="<?php echo $result_empl['user_name']; ?>" /></td>
										<td>Address:</td>
										<td><input type="text" name="empl_address" value="<?php echo $result_empl['empl_address']; ?>" placeholder="Place of Residence" tabindex=6 /></td>
										<td>Employm. End:</td>
										<td>
											<input type="text" name="empl_out" id="datepicker3" placeholder="DD.MM.YYYY"
											<?php if($result_empl['empl_out'] != NULL) echo ' value="'.date("d.m.Y", $result_empl['empl_out']).'"'; ?>
											 tabindex=12 />
										</td>
									</tr>
									<tr>
